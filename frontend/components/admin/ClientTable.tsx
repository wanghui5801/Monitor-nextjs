...
const ClientTable: React.FC<ClientTableProps> = ({ clients, loading, onDelete, onUpdateOrder }) => {
  ...
  const getLinuxCommand = (clientName: string) => {
    ...
    const escapedName = clientName.replace(/['"]/g, '');  // Remove quotes
    return `wget -O install.sh https://raw.githubusercontent.com/wanghui5801/Monitor-nextjs/main/install_client.sh && chmod +x install.sh && sudo ./install.sh '${escapedName}' '${serverIp}'`;
  };

  const getWindowsCommand = (clientName: string) => {
    const serverIp = window.location.hostname;
    const escapedName = clientName.replace(/['"]/g, '');  // Remove quotes
    return `powershell -Command "& { Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/wanghui5801/Monitor-nextjs/main/install_client.bat' -OutFile 'install_client.bat'; Start-Process -FilePath 'install_client.bat' -ArgumentList '${escapedName}','${serverIp}' -Verb RunAs }"`;
  };
